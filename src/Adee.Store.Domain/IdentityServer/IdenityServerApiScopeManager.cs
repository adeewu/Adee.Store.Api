...
{
    public class IdenityServerApiScopeManager : StoreDomainService
    {
            ...
            bool showInDiscoveryDocument)
        {
            ...
            CheckHelper.IsNull(apiScope, $"{name}已存在");

            apiScope = new ApiScope(GuidGenerator.Create(), name, displayName, description, required, emphasize, showInDiscoveryDocument, enabled);
        ...
        {
            var apiScope = await _apiScopeRepository.GetByNameAsync(name, false);
            CheckHelper.IsNotNull(apiScope, $"{name}不存在");

...
{
    public class IdenityServerClientManager : StoreDomainService
    {
        ...
        {
            return _clientRepository.GetListAsync("CreationTime desc", skipCount, maxResultCount, filter, includeDetails, cancellationToken);
        }

        public Task<long> GetCountAsync(string filter = null, CancellationToken cancellationToken = default)
        {
            return _clientRepository.GetCountAsync(filter, cancellationToken);
        }
        ...
        public async Task<Client> CreateAsync(string clientId, string clientName, string description, string allowedGrantTypes)
        {
            ...
            CheckHelper.IsNull(entity, $"{clientId}已存在");
        ...
        )
        {
            ...
            CheckHelper.IsNotNull(client, $"{clientId}不存在");

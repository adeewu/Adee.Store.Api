...
{
    public class IdentityResourceManager : StoreDomainService
    {
        ...
        public Task<IdentityResource> FindByNameAsync(string name, bool includeDetails = true, CancellationToken cancellationToken = default)
        {
            ...
            bool showInDiscoveryDocument)
        {
            ...
            CheckHelper.IsNull(identityResource, $"{name}已存在");

            identityResource = new IdentityResource(GuidGenerator.Create(), name, displayName, description, required, emphasize, showInDiscoveryDocument, enabled);
        ...
        {
            var identityResource = await _identityResourceRepository.FindByNameAsync(name, false);
            CheckHelper.IsNotNull(identityResource, $"{name}不存在");
